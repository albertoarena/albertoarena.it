...
La [documentazione](https://trussphp.com) copre installazione, avvio rapido e autorizzazione per ambienti non locali in maggior dettaglio. Richiede PHP 8.2+ e Laravel 12+. C'è anche una [roadmap](https://trussphp.com/roadmap/) pubblica: il prossimo passo è lo schema diff, per vedere cosa è cambiato dall'ultima migration.
...
Il 27 agosto 2026, Truss ha raggiunto la v1.10.0, abbassando la versione minima di PHP da 8.3 a 8.2, il minimo richiesto da Laravel 12 stesso. Include anche le linee guida e una skill per Laravel Boost, e restringe la regola `TRUSS-INT-007` sulle tabelle pivot dopo averla misurata su schemi reali. Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.10.0).

Il 1° settembre 2026, Truss ha raggiunto la v1.11.0, aggiungendo `TRUSS-INT-010`: segnala una chiave esterna che punta alla tabella sbagliata quando esiste una tabella il cui nome corrisponde alla colonna, lasciando invece intatti i normali alias (come `parent_id` o `variant_id`). Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.11.0).

Il 2 settembre 2026, Truss ha raggiunto la v1.11.1, correggendo un bug nel popover dei dettagli della dashboard: il limite di altezza rientrava in un intervallo che due browser misurano in modo diverso per lo stesso contenuto, così la nota che spiega perché gli export lato server non sono disponibili poteva scorrere fuori vista in un browser ma non nell'altro. Il limite ora si adatta al contenuto più alto che il popover contiene e non può più superare la finestra in cui è posizionato. Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.11.1).

Il 13 settembre 2026, Truss ha raggiunto la v1.12.0, aggiungendo `Truss::payload()`, che restituisce i dati della dashboard (snapshot, diff, report del doctor e flag di cache/baseline) come un semplice array, senza richieste HTTP e senza autorizzazione propria. Il diagramma può ora funzionare con un payload incorporato direttamente nella pagina, in un blocco `<script type="application/json" data-truss-payload>`, con filtri, focus ed etichette dei tipi ancora funzionanti lato client e nessun endpoint dello schema richiesto. Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.12.0).

Il 15 settembre 2026, Truss ha raggiunto la v1.13.0, aggiungendo un interruttore **Show hidden tables** che disegna, in tono attenuato, le tabelle nascoste da `excluded_tables`, controllato da una nuova configurazione `reveal_excluded` che lascia la decisione all'operatore invece che al visitatore. Il footer e `truss:show` ora concordano su quanta parte dello schema è a video (`32 of 40 tables`) invece di essere in disaccordo, e i controlli nativi seguono il tema scelto invece di quello del sistema operativo. Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.13.0).

Il 18 settembre 2026, Truss ha raggiunto la v1.13.1, correggendo il campo Filter della toolbar che collassava fino a sparire quando la dashboard è incorporata in uno spazio più stretto della finestra, come un pannello Filament. I passaggi responsive della barra interrogavano la finestra invece della propria larghezza, così una barra incorporata poteva misurarsi più larga dello spazio realmente disponibile, e tutto il disavanzo ricadeva sul campo Filter. I passaggi ora seguono la larghezza propria della toolbar. Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.13.1).

Il 19 settembre 2026, Truss ha raggiunto la v1.14.0, aggiungendo un formato di export `html`: `php artisan truss:export --format=html --output=schema.html` scrive l'intera dashboard, diagramma, filtro, selettore di focus, pannello structure health, zoom e pan, in un unico file che si apre con un doppio clic, senza server, rete o database. `--mermaid=cdn` sostituisce la copia di Mermaid inclusa nel file con un caricamento da CDN, riducendo il file da megabyte a kilobyte, al prezzo di richiedere una rete per aprirlo. Il formato è disponibile solo da riga di comando: la rotta di export protetta e gli strumenti MCP continuano a servire i sei formati testuali. Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.14.0).

Il 25 settembre 2026, Truss ha raggiunto la v1.14.1, correggendo il formato di export `html` in modo che escapi `<` e `>` nel payload che incorpora, così uno schema con nomi o valori predefiniti di colonna che contengono un `</script>` di chiusura non può più uscire dal blocco JSON incorporato. Prima di questa correzione un valore simile troncava silenziosamente il file esportato, o lasciava che il resto del valore venisse interpretato come markup. Solo il formato HTML era interessato; gli altri sei formati di export non sono cambiati. Vedi le [note di rilascio](https://github.com/albertoarena/laravel-truss/releases/tag/v1.14.1).
