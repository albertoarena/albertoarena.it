...
On September 2, 2026, Truss reached v1.11.1, fixing a bug in the dashboard's details popover: its height cap sat inside a range two browsers measure differently for the same content, so the note explaining why server-backed exports are unavailable could scroll out of view in one browser but not the other. The cap now clears the tallest content the popover carries and can no longer exceed the window it's placed in. See the [release notes](https://github.com/albertoarena/laravel-truss/releases/tag/v1.11.1).

On September 13, 2026, Truss reached v1.12.0, adding `Truss::payload()`, which returns the dashboard's data (snapshot, diff, doctor report, and cache/baseline flags) as a plain array with no HTTP request and no authorization of its own. The diagram can now run against a payload embedded directly in the page, in a `<script type="application/json" data-truss-payload>` block, with filtering, focus, and type labels still working client-side and no schema endpoint required. See the [release notes](https://github.com/albertoarena/laravel-truss/releases/tag/v1.12.0).

On September 15, 2026, Truss reached v1.13.0, adding a **Show hidden tables** toggle that draws the tables `excluded_tables` hides, muted, controlled by a new `reveal_excluded` config that keeps the decision on the operator rather than the viewer. The footer and `truss:show` now both report how much of the schema is on screen (`32 of 40 tables`) instead of disagreeing about it, and native controls follow the chosen theme instead of the operating system's. See the [release notes](https://github.com/albertoarena/laravel-truss/releases/tag/v1.13.0).

On September 18, 2026, Truss reached v1.13.1, fixing the toolbar's Filter field collapsing to nothing when the dashboard is embedded in something narrower than the window, such as a Filament panel. The bar's responsive steps asked the viewport rather than its own width, so an embedded bar could measure wider than the space it actually had, and the whole shortfall landed on the Filter field. The steps now follow the toolbar's own width instead. See the [release notes](https://github.com/albertoarena/laravel-truss/releases/tag/v1.13.1).

On September 19, 2026, Truss reached v1.14.0, adding an `html` export format: `php artisan truss:export --format=html --output=schema.html` writes the whole dashboard, diagram, filter, focus picker, structure health panel, zoom and pan, into a single file that opens with a double-click, with no server, network, or database needed. `--mermaid=cdn` swaps the bundled copy of Mermaid for a CDN load, shrinking the file from megabytes to kilobytes at the cost of needing a network to open it. The format is CLI-only: the gated export route and the MCP tools continue to serve the six text formats. See the [release notes](https://github.com/albertoarena/laravel-truss/releases/tag/v1.14.0).

On September 25, 2026, Truss reached v1.14.1, fixing the `html` export format to escape `<` and `>` in the payload it embeds, so a schema whose names or column defaults contain a closing `</script>` can no longer break out of the embedded JSON block. Before this fix such a value silently truncated the exported file or let the remainder be parsed as markup. Only the HTML format was affected; the other six export formats were unchanged. See the [release notes](https://github.com/albertoarena/laravel-truss/releases/tag/v1.14.1).
